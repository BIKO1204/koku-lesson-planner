...
export default async function AdminUsersPage() {
  ...
  return (
    <main style={{ maxWidth: 900, margin: "40px auto", padding: "0 16px" }}>
      <h1 style={{ fontSize: "1.5rem", marginBottom: 16 }}>ユーザー一覧</h1>
      ...
      <div style={{ overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr>
                <td colSpan={3} style={{ padding: 12, color: "#666" }}>ユーザーがまだいません。</td>
              </tr>
            ) : (
              rows.map((r) => (
                <tr key={r.id}>
                  <td style={{ padding: 8, borderBottom: "1px solid #eee" }}>{r.name || "（未設定）"}</td>
                  <td style={{ padding: 8, borderBottom: "1px solid #eee" }}>{r.email}</td>
                  <td style={{ padding: 8, borderBottom: "1px solid #eee" }}>{fmtJST(r.lastLogin)}</td>
                </tr>
              ))
            )}
          </tbody>
